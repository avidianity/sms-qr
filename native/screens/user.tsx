		...
		<Fragment>
			<Overlay isVisible={!!openDate} onBackdropPress={() => setOpenDate(undefined)}>
				...
				<Text>
					...
					{openDate?.attendance.createdAt && new Date(openDate?.attendance.createdAt).toTimeString()}
				</Text>
				<Text>
					<Text style={{ fontWeight: 'bold' }}>Time of Update:</Text>{' '}
					{openDate?.attendance.updatedAt && new Date(openDate?.attendance.updatedAt).toTimeString()}
				</Text>
			...
			<View style={{ flex: 1 }}>
				<ScrollView refreshControl={<RefreshControl refreshing={isLoading} onRefresh={() => refetch()} />}>
					...
					<Calendar markingType='custom' markedDates={dates} onDayPress={handleDayPress} />
				</ScrollView>
			</View>
		</Fragment>
	);
