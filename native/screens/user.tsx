...
export function UserScreen(
  props: NativeStackScreenProps<RootStackParamList, "User">
) {
  const user = props.route?.params?.user;

  const { data, isLoading, refetch } = useQuery(
    ["attendance", user?.id],
    async () => await getAttendance(user)
  );
  ...
  const handleDayPress = (date: DateObject) => {
    if (dates[date.dateString]) {
      const attendance = data?.data.find((data) =>
        data.createdAt.startsWith(date.dateString) ? data : null
      );
  ...
  return (
    <Fragment>
      <Overlay
        isVisible={!!openDate}
        onBackdropPress={() => setOpenDate(undefined)}
      >
        <Text style={{ fontSize: 20, fontWeight: "bold" }}>
          Attendance on {openDate?.dateString}
        </Text>
        <Divider style={{ marginVertical: 16 }} />
        <Text>
          <Text style={{ fontWeight: "bold" }}>Time of Creation:</Text>{" "}
          {openDate?.attendance.createdAt &&
            new Date(openDate?.attendance.createdAt).toTimeString()}
        </Text>
        <Text>
          <Text style={{ fontWeight: "bold" }}>Time of Update:</Text>{" "}
          {openDate?.attendance.updatedAt &&
            new Date(openDate?.attendance.updatedAt).toTimeString()}
        </Text>
        <View style={{ alignItems: "flex-end", paddingTop: 16 }}>
          <Button title="Close" onPress={() => setOpenDate(undefined)} />
        </View>
      </Overlay>
      <View style={{ flex: 1 }}>
        <ScrollView
