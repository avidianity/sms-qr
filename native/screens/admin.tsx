...
export function AdminScreen(props: NativeStackScreenProps<RootStackParamList, 'Admin'>) {
	...
	const DownloadAttendance = async () => {
		if (user) {
			FileSystem.downloadAsync(SERVER_API + '/attendances/attendances', FileSystem.documentDirectory + 'attendance.xlsx', {
				headers: {
					...
					FileSystem.getContentUriAsync(res.uri)
						.then((cUri) => {
							IntentLauncher.startActivityAsync('android.intent.action.VIEW', {
								...
								ToastAndroid.show('No app can be used to open the file!', ToastAndroid.SHORT);
							});
						})
						.catch((err: Error) => {
							ToastAndroid.show('Cannot open file! ' + err.message, ToastAndroid.SHORT);
						});
				})
				.catch((err: Error) => {
					ToastAndroid.show('Cannot open file! ' + err.message, ToastAndroid.SHORT);
