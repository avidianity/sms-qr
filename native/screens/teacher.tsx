...
export function TeacherScreen(
  props: NativeStackScreenProps<RootStackParamList, "Teacher">
) {
  useKeepAwake();
  const { data, logout } = useAuth(props);
  const [avatarColor] = useState(randomColor({ luminosity: "dark" }));

  const { data: qr, isSuccess } = useQuery(
    "teacher_qr",
    async () => await getQR(data?.data.user),
    {
        ...
        <LinearGradient colors={["#f37335", "#c25c2a"]} style={{ padding: 24 }}>
          ...
          <View>
            <Text style={{ marginBottom: 8, color: "white" }}>
              Present your QR code to authorized personnels to have your
              attendance.
            </Text>
          </View>
        </LinearGradient>
